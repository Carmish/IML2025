...
def plot_results(Y_test, Y_pred, errors_nn, losses, val_losses, model_name=None, training_time=None):
    ...
    Gives three subplots:
    1. Scatter plot of true vs predicted outputs with error vectors.
        Visualizes how close the neural network predictions are to the true targets (of the test set).
        The blue points represent the true target values, while the green points represent the neural network predictions.
        The arrows indicate the direction and magnitude of the errors between the predictions and the true targets.
    2. Training and validation loss curve over epochs. 
        Shows how the model's loss changes during training and validation, helping to identify overfitting or underfitting.
        Overfitting will be indicated by a significant gap between training and validation loss curves (with training loss decreasing while validation loss increases).
        Underfitting will be indicated by both training and validation loss remaining high.
    3. Histogram of absolute errors for neural network predictions.
        Displays the distribution of absolute errors (of the test set predictions) in the x and y coordinates, helping to understand the error magnitude and frequency.
        We can see how many predictions fall within certain error ranges (far from the true target values, or close to them) for both x and y coordinates.
